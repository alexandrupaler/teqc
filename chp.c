...
void printstate_paler(const char* fname, struct QState *q)

// Print the destabilizer and stabilizer for state q

{
	char specfilename[255];
	sprintf(specfilename, "%s.spec", fname);
	FILE* file = fopen(specfilename, "w");
	...
	for (i = q->n; i < 2*q->n; i++)
	{
		//informatia despre faza nu o tiparesc
         //if (q->r[i]==2) printf("\n-");
         //else printf("\n+");
		int isStart = 0;
		int isX = 0;
		for (j = 0; j < q->n; j++)
		{
			j5 = j>>5;
			pw = q->pw[j&31];
			//if ((!(q->x[i][j5]&pw)) && (!(q->z[i][j5]&pw)))//identity
				//printf("");
			if ((q->x[i][j5]&pw) && (!(q->z[i][j5]&pw)))//X
			{
				isX = 1;
				if(isStart == 0)
					fprintf(file, "X:");
				if(isStart != 0)
					fprintf(file, ",");
				isStart +=1;
				fprintf(file, "%ldA", j+1);
			}
			if ((q->x[i][j5]&pw) && (q->z[i][j5]&pw))//Y
				printf("Y");//deocamdata nu am Y-uri
			if ((!(q->x[i][j5]&pw)) && (q->z[i][j5]&pw))//Z
			{
				if(isStart == 0)
					fprintf(file, "X: Z:");
				if(isStart != 0)
...

{
	...
	for (i = 0; i < 2*q->n; i++)
	{
         ...
         for (j = 0; j < q->n; j++)
         {
                 j5 = j>>5;
                 pw = q->pw[j&31];
                 if ((!(q->x[i][j5]&pw)) && (!(q->z[i][j5]&pw)))         printf(" ");
...

{
	...
	printf("\nCHP Simulator adapted to print ICM specification");
	srand(time(0));
	if (argc==1) error(0);
	if (argv[1][0]=='-') param = 1;
	h = (struct QProg*) malloc(sizeof(struct QProg));
	q = (struct QState*) malloc(sizeof(struct QState));
